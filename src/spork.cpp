...
// Scamar: on startup load spork values from previous session if they exist in the sporkDB
	...
	if (strCommand == "spork") {
		...
		if (spork.nTimeSigned >= Params().NewSporkStart()) {
			if (!sporkManager.CheckSignature(spork, true)) {
				...
				Misbehaving(pfrom->GetId(), 100, 40);
				return;
			}
		}
		...
		if (!sporkManager.CheckSignature(spork)) {
			LogPrintf("%s : Invalid Signature\n", __func__);
			Misbehaving(pfrom->GetId(), 100, 41);
			return;
		}
