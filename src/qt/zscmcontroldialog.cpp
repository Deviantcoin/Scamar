...
#include "ui_zscmcontroldialog.h"
...
std::set<std::string> ZScmControlDialog::setSelectedMints;
std::set<CMintMeta> ZScmControlDialog::setMints;

ZScmControlDialog::ZScmControlDialog(QWidget *parent) :
	QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
	ui(new Ui::ZScmControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZScmControlDialog::~ZScmControlDialog()
{
    delete ui;
}

void ZScmControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZScmControlDialog::updateList()
{
...
void ZScmControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZScmControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZScm_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZScmControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZScmControlDialog::GetSelectedMints()
{
...
void ZScmControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
