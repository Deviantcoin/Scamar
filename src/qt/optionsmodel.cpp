...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::SCM);
    ...
    if (!settings.contains("nAnonymizeScamarAmount"))
        settings.setValue("nAnonymizeScamarAmount", 1000);

    nAnonymizeScamarAmount = settings.value("nAnonymizeScamarAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeScamarAmount"))
        SoftSetArg("-anonymizescamaramount", settings.value("nAnonymizeScamarAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in scamar.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeScamarAmount:
            return QVariant(nAnonymizeScamarAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeScamarAmount:
            nAnonymizeScamarAmount = value.toInt();
            settings.setValue("nAnonymizeScamarAmount", nAnonymizeScamarAmount);
            emit anonymizeScamarAmountChanged(nAnonymizeScamarAmount);
            break;
        case CoinControlFeatures:
