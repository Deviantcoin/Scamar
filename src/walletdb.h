...
public:
    ...
    bool WriteZSCMSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSCMSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSCMSeed_deprecated(uint256& seed);
    bool EraseZSCMSeed();
    bool EraseZSCMSeed_deprecated();

    bool WriteZSCMCount(const uint32_t& nCount);
    bool ReadZSCMCount(uint32_t& nCount);
