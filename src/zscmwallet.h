...
#ifndef SCAMAR_ZSCMWALLET_H
#define SCAMAR_ZSCMWALLET_H
...
public:
    CzSCMWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZSCM(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZSCM(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //SCAMAR_ZSCMWALLET_H
