...
CzSCMWallet::CzSCMWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zscm seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSCMSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSCMSeed_deprecated()) {
                    LogPrintf("%s: Updated zSCM seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zscm seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zscm wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSCMWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSCMCount(nCountLastUsed);
    else if (!walletdb.ReadZSCMCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSCMWallet::Lock()
{
    seedMaster = 0;
}

void CzSCMWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSCMWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSCM(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSCMWallet::LoadMintPoolFromDB()
{
...
void CzSCMWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSCMWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSCMWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zscmTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSCMWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSCM(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zscmTracker which also adds to database
    pwalletMain->zscmTracker->Add(dMint, true);
...
void CzSCMWallet::SeedToZSCM(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSCMWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSCMWallet::GenerateDeterministicZSCM(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSCMWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSCMWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
