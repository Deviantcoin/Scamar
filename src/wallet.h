...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT25000IFMN = 3,
    ONLY_NONDENOMINATED_NOT25000IFMN = 4, // ONLY_NONDENOMINATED and not 25000 SCM at the same time
    ONLY_25000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zSCM send
enum ZerocoinSpendStatus {
    ZSCM_SPEND_OKAY = 0,                            // No error
    ZSCM_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZSCM_WALLET_LOCKED = 2,                         // Wallet was locked
    ZSCM_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZSCM_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZSCM_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZSCM_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZSCM_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZSCM_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZSCM_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZSCM_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZSCM_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZSCM_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZSCM_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZSCM_SPENT_USED_ZSCM = 14,                      // Coin has already been spend
    ZSCM_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZSCM_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZScmBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZSCMOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzscmAuto) const;
    ...
    */
    ...
    CzSCMWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzSCMTracker> zscmTracker;
    ...
    void setZWallet(CzSCMWallet* zwallet)
    {
        zwalletMain = zwallet;
        zscmTracker = std::unique_ptr<CzSCMTracker>(new CzSCMTracker(strWalletFile));
    }

    CzSCMWallet* getZWallet() { return zwalletMain; }
    ...
    void setZScmAutoBackups(bool fEnabled)
    {
    ...
    /** zSCM reset */
    boost::signals2::signal<void()> NotifyzSCMReset;
